...
本文章摘自掘金
作者：CUGGZ
链接：[掘金原文](https://juejin.cn/post/6919373017218809864)
:::

![Vue面试题.png](./vue.assets/2a44f18af182434d901b74df18491dea~tplv-k3u1fbpfcp-zoom-1.png)

## 一、Vue 基础

### 1. Vue的基本原理

当一个Vue实例创建时，Vue会遍历data中的属性，用 Object.defineProperty（vue3.0使用proxy ）将它们转为 getter/setter，并且在内部追踪相关依赖，在属性被访问和修改时通知变化。 每个组件实例都有相应的 watcher 程序实例，它会在组件渲染的过程中把属性记录为依赖，之后当依赖项的setter被调用时，会通知watcher重新计算，从而致使它关联的组件得以更新。 ![img](./vue.assets/b1b16025a35b4cd2b343a92e740621b7~tplv-k3u1fbpfcp-zoom-1.png)
...
![img](./vue.assets/a286bdc076ae425fb9591bb8c4153240~tplv-k3u1fbpfcp-zoom-1.png)
...
这种模式实现了 Model和View的数据自动同步，因此开发者只需要专注于数据的维护操作即可，而不需要自己操作DOM。 ![image.png](./vue.assets/d5ce15b7b704483eb91ee1f5d1d64786~tplv-k3u1fbpfcp-zoom-1.png)
...
在Vue中，对响应式处理利用的是Object.defineProperty对数据进行拦截，而这个方法并不能监听到数组内部变化，数组长度变化，数组的截取变化等，所以需要对这些操作进行hack，让Vue能监听到其中的变化。 ![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6be20296a76f4e7085dfb510a3211d16~tplv-k3u1fbpfcp-zoom-1.png) 那Vue是如何实现让这些数组方法实现元素的实时更新的呢，下面是Vue中对这些方法的封装：
...
**区别：** ![775316ebb4c727f7c8771cc2c06e06dd.jpg](./vue.assets/76b3d747986e45e096abaf64faf5e332~tplv-k3u1fbpfcp-zoom-1.png)
...
![bb253b1d177f421741af0e7dd0f52b5e.jpg](./vue.assets/5e7df745017242a7beaba81e854a0f97~tplv-k3u1fbpfcp-zoom-1.png) **（2)mergeOptions 的执行过程**
